...
const SocialTrendsPage: React.FC<SocialTrendsPageProps> = ({ savedTokens, onSave, onUnsave }) => {
    ...
    const handleScan = useCallback(async () => {
        ...
        try {
            const { tokens: foundTokens, sources: foundSources } = await findSocialTrends();
            setTokens(foundTokens);
            setSources(foundSources);
            setScanTime(new Date());
        } catch (err) {
            setError(err instanceof Error ? err.message : 'An unknown error occurred while scanning social trends.');
            console.error(err);
        } finally {
            setIsLoading(false);
        }
